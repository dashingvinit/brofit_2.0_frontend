...
export function CreateMembershipForm({
  ...
  const { data: planTypes, isLoading: planTypesLoading } =
    usePlanTypesByCategory(mode);
  const { data: trainingPlanTypes, isLoading: trainingPlanTypesLoading } =
    usePlanTypesByCategory("training");
  ...
  const steps = useMemo(() => {
    if (mode === "membership" && addTraining)
  ...
  const onSubmit = (data: CreateMembershipFormData) => {
    ...
    if (mode === "training") {
      const trainingPayload: CreateTrainingData = {
        ...
        notes: data.notes,
        trainerFixedPayout:
          data.trainerFixedPayout != null ? data.trainerFixedPayout : undefined,
    ...
    const totalDue =
      membershipFinalPrice + (data.addTraining ? trainingFinalPriceVal : 0);
    ...
    if (data.collectPayment && data.paymentAmount && data.paymentMethod) {
      if (data.addTraining && trainingFinalPriceVal > 0 && totalDue > 0) {
        membershipPaymentAmount = Math.round(
          (data.paymentAmount * membershipFinalPrice) / totalDue,
        );
    ...
    const payload: CreateMembershipData = {
      ...
      trainingPlanVariantId: data.addTraining
        ? data.trainingPlanVariantId
        : undefined,
      ...
      >
          ...
          <MemberPicker
            ...
            activeWarning={
              existingActiveMembership
                ? { type: "membership", item: existingActiveMembership }
                : null
            }
          />
        )}
        ...
        {currentStepId === "details" && (
          <MembershipDetailsStep
            ...
            onToggleTraining={
              mode === "membership" ? handleToggleTraining : undefined
            }
            mode={mode}
            trainers={trainers}
